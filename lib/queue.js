...
module.exports = (function(name, opts) {
    ...
    this.push = function(task, fn) {
        ...
        redis.rpush("redisq:queue:" + self.name, task.stringify(), function(err, res) {
    ...
    this.len = function(fn) {
        ...
        redis.llen("redisq:queue:" + self.name, function(err, length) {
            if (err) throw new Error(err);
    ...
    this.purge = function(fn) {
        redis.del("redisq:queue:" + self.name, function(err, res) {
            if (err) throw new Error(err);
            if (fn)  fn(null, true);
        });
    };
    ...
    function _spawn() {
        redis.lpop("redisq:queue:" + self.name, function(err, tdata) {
            if (err) throw new Error(err);
    ...
    this.register = function(fn) {
        redis.hset("redisq:queues", self.name, 1, fn);
    };

    this.unregister = function(fn) {
        redis.hdel("redisq:queues", self.name, fn);
    };
